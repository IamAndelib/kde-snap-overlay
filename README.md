...
Grab `kde-snap-overlay-1.6.0.kwinscript` from the repo root or the releases page (also published on the [KDE Store](https://store.kde.org)):
...
| `overlayFadeIn`      | 200     | 0–1000 | Fade-in duration (ms) of the zone overlay — FancyZones' 200ms linear alpha ramp |
| `overlayFadeOut`     | 150     | 0–500 | Fade-out duration (ms) when the overlay disengages (FancyZones hides instantly; the soft retreat is our refinement) |
...
- The overlay follows **FancyZones' (MIT) animation model**: the popup cards highlight instantly, but the fullscreen zone overlay only engages after the cursor rests `highlightDelay` on one layout — sweeping across the cards never pops the overlay window — its show is a 200ms linear fade (FancyZones' `FadeInDurationMillis`), disengage plays a soft fade-out instead of an instant hide, and the fill is drawn at `highlightOpacity` (FancyZones' default 50%). Drop semantics are untouched: a quick flick-and-drop still snaps from the *instant* hover zone, the dwell gates only the visuals.
